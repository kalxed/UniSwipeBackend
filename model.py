...
class CollegeRecommender:
    def __init__(self):
        ...
        self.colleges = pd.read_csv("data/collegedata.csv")  # Ensure you have a CSV file with the required data
        self.q_table = {}
        ...
        self.colleges.dropna(subset=["INSTNM", "CITY", "STABBR", "ZIP", "INSTURL", "ADM_RATE"], inplace=True)
    ...
    def update(self, state, action, reward):
        ...
        if self.last_state_action is not None:
            ...
            self.q_table[last_state_action] = self.q_table.get(last_state_action, 0) + \
                                              self.alpha * (reward + self.gamma * self.q_table[state_action] - self.q_table[last_state_action])
